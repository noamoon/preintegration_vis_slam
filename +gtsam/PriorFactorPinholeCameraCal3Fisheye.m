...
classdef PriorFactorPinholeCameraCal3Fisheye < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PriorFactorPinholeCameraCal3Fisheye(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2233, varargin{2});
        end
        base_ptr = gtsam_wrapper(2232, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.PinholeCameraCal3Fisheye') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2234, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2235, obj.ptr_gtsamPriorFactorPinholeCameraCal3Fisheye);
    end
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2238, varargin{:});
      else
