...
classdef KarcherMeanFactorPose3 < gtsam.NonlinearFactor
  ...
  methods
    function obj = KarcherMeanFactorPose3(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2952, varargin{2});
        end
        base_ptr = gtsam_wrapper(2951, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'gtsam.KeyVector')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2953, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2954, obj.ptr_gtsamKarcherMeanFactorPose3);
    end
